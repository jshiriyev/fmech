...
class Cruncher:
	...
	def DDI(initial:Model,model:Model):
		...
		return initial.reservoir.N*(Cruncher.Btotal(initial,model)-initial.phase.Bo)/Cruncher.Ntotal(model)

	@staticmethod
	def SDI(initial:Model,model:Model):
		"""Segregation (gas-cap) drive index"""
		return initial.reservoir.N*Cruncher.M(initial)*initial.phase.Bo*(model.phase.Bg-initial.phase.Bg)/(initial.phase.Bg*Cruncher.Ntotal(model))
	...
	def EDI(initial:Model,model:Model):
		...
		return Cruncher.PV(initial)*(model.phase.cf+model.phase.cw*initial.reservoir.Sw)*(initial.reservoir.P-model.reservoir.P)/Cruncher.Ntotal(model)